...
char_freq = compute_probabilities(input_str)

# Create tree nodes for each symbol
tree_nodes = [TreeNode(freq, char) for char, freq in char_freq.items()]

# Convert tree nodes to a list of tuples of the symbol and the freq
node_tuples = [(node.char, node.freq) for node in tree_nodes]
print("Node tuples:", node_tuples)

# Build the Huffman tree
huffman_tree_root = huffman_encoding(input_str)[1]

# Calculate the Huffman codes
huffman_codes = compute_char_codes(huffman_tree_root)
print("Huffman codes:", huffman_codes)

# Encode the input string
encoded_str = encode_output(input_str, huffman_codes)
