...
def encrypt(KEY, plaintext):
    ...
    for i in range(len(plaintext_vectors), 2):
        pt_column_vectors.append([plaintext_vectors[i][0] + plaintext_vectors[i+1][0]])
        pt_column_vectors.append([plaintext_vectors[i][1] + plaintext_vectors[i+1][1]])
    
    # Convert column vectors to encoded vectors
    for i in range(len(pt_column_vectors)):
        for j in range(2):
            for key in mapping_scheme:
                if pt_column_vectors[i][j] == key:
    ...
    for column in range(len(ciphertext_array)):
        ciphertext.append(ciphertext_array[column][0].tolist())
        ciphertext.append(ciphertext_array[column+1][0].tolist())
